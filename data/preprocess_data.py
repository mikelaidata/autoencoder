...
def get_dataset_1M(ROOT_DIR):
    ...
    gc.enable()
    training_set = pd.read_csv(os.path.join(ROOT_DIR, 'ml-1m/train.dat'), sep='::', header=None, engine='python', encoding='latin-1')
    training_set = np.array(training_set, dtype=np.uint32)

    test_set = pd.read_csv(os.path.join(ROOT_DIR, 'ml-1m/test.dat'), sep='::', header=None, engine='python', encoding='latin-1')
